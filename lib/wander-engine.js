...
function furnish(group, type, cx, cz, w, d, ang0, M, H) {
  ...
  const rug = (rw,rd,col,x=0,z=0) => { const r=new THREE.Mesh(new THREE.PlaneGeometry(rw,rd), new THREE.MeshStandardMaterial({map:fabric(col),roughness:1})); r.rotation.x=-Math.PI/2; r.position.set(x,0.012,z); r.receiveShadow=true; return add(r); };
  const warm = (x,y,z,i=3,dist=4) => { const l=new THREE.PointLight(0xffd2a0,i,dist,2); l.position.set(x,y,z); add(l); };
  const pic = (key,x,y,z,ry,sc=1) => add(asset(key,x,z,ry,{y, scale:sc}));
  ...
  if (type==='living') {
    ...
    pic('picture2', -1.25, 1.55, back+0.09, 0, 1.0); if (w>4) pic('picture', 1.3, 1.5, back+0.09, 0, 0.9);
    ...
    add(rounded(bw+0.16,0.16,1.45,0.08,M.duvet)).position.set(0,0.52,back+1.3); add(box(bw*0.42,0.14,0.4,M.linen,-bw*0.25,0.62,back+0.5)); add(box(bw*0.42,0.14,0.4,M.linen,bw*0.25,0.62,back+0.5));
    [-1,1].forEach(sd=>{ if (w > bw+1.2) { add(asset('nightstand', sd*(bw/2+0.4), back+0.32, 0, {w:0.5})); add(asset('vase', sd*(bw/2+0.4), back+0.32, 0, {h:0.3, y:0.62})); warm(sd*(bw/2+0.4), 0.9, back+0.32, 2.5, 3); } });
    if (w>3.2) add(asset('wardrobe', right-0.35, 0.4, -Math.PI/2, {h:1.9, y:0}));
    add(asset('plant3', left+0.3, front-0.3, 0, {h:0.3})); if (mid) add(asset('plant', left+0.4, front-0.5, 0, {h:1.2}));
    pic('picture3', 0.3, 1.75, back+0.09, 0, 1.0); if (w>3) add(asset('mirror', right-0.02, front-0.9, -Math.PI/2, {y:1.4, h:0.8}));
...
export function buildHouse(scene, rooms, opts) {
  ...
  rooms.forEach((room, ri) => {
    ...
    const ceil = new THREE.Mesh(new THREE.ShapeGeometry(shape), M.ceiling); ceil.rotation.x = Math.PI/2; ceil.position.y = H; ceil.userData.ceiling = true; house.add(ceil);
    // wall paint per room
    ...
    if (type!=='garage' && sz.x*sz.y>2) { const l=new THREE.PointLight(0xffe4c4, type==='hall'?2.5:5, Math.max(sz.x,sz.y)*1.6, 1.8); l.position.set(c.x, H-0.25, c.y); l.castShadow = quality==='full' && ri<4; l.shadow.mapSize.set(512,512); l.shadow.bias=-0.004; house.add(l);
      const lamp=(type==='kitchen'&&sz.x>3.6&&sz.y>3.6)?null:asset('ceiling_lamp', c.x, c.y, 0, {h:0.5, y:H-0.5-0.22*(0.5/0.95)}); if (lamp) house.add(lamp); else house.add(cyl(0.16,0.03,M.white,c.x,H-0.02,c.y)); }
    // walls
    roomSegs[ri].forEach(([a,b]) => {
      const k = edgeKey(a,b); if (built.has(k)) return; built.add(k);
      const interior = edgeCount[k] > 1 && !edgeTypes[k].includes('garage') && type!=='garage';
  ...
  const roof = new THREE.Mesh(new THREE.BoxGeometry(size.x+0.6, 0.25, size.z+0.6), M.matte(0x4a4a4a)); roof.position.set(centre.x, H+0.13, centre.z); roof.castShadow=true; roof.userData.ceiling=true; scene.add(roof);
...
export function createViewer(canvas, overlay) {
  ...
  return {
    ...
    async load(rooms, opts) {
      ...
      camera.position.set(pos[0], 1.5, pos[1]); camera.rotation.set(-0.03, Math.atan2(-(tgt[0]-pos[0]), -(tgt[1]-pos[1])), 0);
      resize(); return built;
    },
    teleport(c){ doll=false; setCeilings(true); camera.position.set(c.x,1.62,c.z); camera.rotation.x=0; },
    ...
    glideTo(c, ms=900){ const from=camera.position.clone(); const to=new THREE.Vector3(c.x,1.62,c.z); const t0=performance.now(); doll=false; setCeilings(true); const step=()=>{ const k=Math.min(1,(performance.now()-t0)/ms); const e=k<.5?2*k*k:1-Math.pow(-2*k+2,2)/2; camera.position.lerpVectors(from,to,e); camera.rotation.x*= (1-e*0.1); if(k<1) requestAnimationFrame(step); }; step(); },
