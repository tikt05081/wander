...
function furnish(group, type, cx, cz, w, d, ang0, M, H) {
  ...
  if (type==='living') {
    ...
    if (w>4.2) pic('picture2', 1.45, 1.55, back+0.09, 0, 1.0);
...
export function buildHouse(scene, rooms, opts) {
  ...
  function buildWall(a,b,interior,mat,type) {
    ...
    if (interior && len>1.3) {           // doorway near one end of the wall (like a real house), with architrave + open leaf
      ...
      { const wl = new THREE.PointLight(0xdfe9ff, 1.4, Math.max(4, len*1.4), 1.6); const nx=-Math.sin(ang), nz=Math.cos(ang); const inside=((centreRef.x-mid.x)*nx+(centreRef.z-mid.z)*nz)>0?1:-1; wl.position.set(mid.x+nx*inside*0.35, sill+gh*0.6, mid.z+nz*inside*0.35); house.add(wl); }
  ...
  const roof = new THREE.Mesh(new THREE.BoxGeometry(size.x+0.6, 0.25, size.z+0.6), M.matte(0x4a4a4a)); roof.position.set(centre.x, H+0.13, centre.z); roof.castShadow=false; roof.userData.ceiling=true; scene.add(roof);
...
export function createViewer(canvas, overlay) {
  ...
  function safeSpot(ri){ const rb=builtRef.roomBounds[ri], rc=builtRef.roomCentres[ri]; const o=builtRef.orient[ri]||{back:0}; const W=rb.max.x-rb.min.x, D=rb.max.y-rb.min.y;
    ...
    const cands=[corner]; for (let r=0.4; r<Math.max(W,D); r+=0.4) for (let a=0;a<Math.PI*2;a+=Math.PI/6) cands.push([corner[0]+Math.cos(a)*r, corner[1]+Math.sin(a)*r]);
    for (const [x,z] of cands){ if (x<rb.min.x+0.4||x>rb.max.x-0.4||z<rb.min.y+0.4||z>rb.max.y-0.4) continue; if (!blocked(x,z)) return {x,z,yaw:Math.atan2(-(tgt[0]-x), -(tgt[1]-z))}; }
  ...
  return {
    ...
    async load(rooms, opts) {
      ...
      const sun = new THREE.DirectionalLight(0xfff1dc, 2.6); sun.position.set(12, 16, 9); sun.castShadow = true; sun.shadow.mapSize.set(2048,2048); sun.shadow.bias = -0.0008; sun.shadow.normalBias = 0.02;
